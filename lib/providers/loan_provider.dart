...
class UserModel {
  final String fullName, mobile, pan, dob, upiId,
      address, city, state, pincode, faceImagePath;
  ...
  Map<String, dynamic> toJson() => {
    ...
    'pincode': pincode, 'faceImagePath': faceImagePath, 'allImages': allImages,
  };
...
class AppProvider extends ChangeNotifier {
  ...
  bool get isBlacklisted =>
      _loans.any((l) => l.status == LoanStatus.blacklisted);
  ...
  Future<UserModel?> _findUserByMobile(String mobile) async {
    try {
      ...
      return found.isEmpty ? null : UserModel.fromJson(found.first);
    } catch (_) { return null; }
  }
  ...
  Future<void> updateLoanPdf(String loanId, String path) async {
    ...
    if (i >= 0) { _loans[i].pdfPath = path; await _saveLoans(); notifyListeners(); }
  }
  ...
  Future<void> deleteAccount() async {
    try {
      ...
      if (await f.exists()) {
        List users = jsonDecode(await f.readAsString()) as List;
        users.removeWhere((u) => u['mobile'] == _user?.mobile);
        await f.writeAsString(jsonEncode(users));
      }
    ...
    final p = await SharedPreferences.getInstance();
    await p.clear();
    _user = null; _loggedIn = false;
    _onTimePayments = 0; _loans = [];
    notifyListeners();
  }
}
